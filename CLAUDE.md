...
- Discover: `GET /discover?sport=TENNIS|RUNNING` — excludes already-swiped users and filters by the caller's `ageMin`/`ageMax`, `distanceKm` and `genderPreference` preferences (age falls back to 18-60 if no preferences row yet; distance via Haversine against `Profile.latitude/longitude`, no PostGIS). Two deliberate "don't punish missing data" carve-outs: neither viewer nor candidate missing coordinates is affected by the distance filter, and a candidate who hasn't stated a gender is never excluded by the gender filter — silently emptying someone's feed the moment they set a preference is a worse failure than showing a few unlabelled profiles. **Which sport to ask for is the client's call**, taken from `Preferences.sportsWanted` (falling back to the user's own `Profile.sports`) — the endpoint itself just honours the `?sport=` it's given.
...
- Photos are always stored and shown in 16:9 landscape. `core/utils/landscape_crop.dart` centre-crops with `dart:ui` (no extra dependency; same code path on mobile and web, and it uses the GPU instead of walking pixels in Dart) and re-encodes to PNG, so the uploaded filename/content-type must be set to `.png`/`image/png` rather than the picker's originals. Both upload entry points (onboarding and the photo manager sheet) go through `showPhotoCropPreview` first, so the user approves the exact framing before it uploads.
