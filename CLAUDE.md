...
- The launcher icon is **derived from `tool/logo_source.png` by code, not exported from an image editor**: `tool/gen_app_icon.dart` reads that logo and writes the three PNGs under `assets/icon/` — `app_icon.png` (opaque; iOS/web/legacy Android), plus `app_icon_background.png` / `app_icon_foreground.png`, the two Android adaptive-icon layers. It decodes and encodes PNG by hand (`dart:io` already ships zlib), so no image package is involved; same reasoning as `datagen`'s generated seed photos. It recolours the source's two halves to the app's own per-sport colours from `core/utils/sport_words.dart` and swaps the white surround for the slate `secondary`; the artwork mask comes from projecting each pixel onto the background→white line, which inherits the source's antialiasing instead of rebuilding it with a threshold. `AppLogo` (Welcome, login/register) renders `app_icon.png` itself, so the launcher icon and the in-app logo can't drift apart. **Gotcha — the adaptive layers are not the same scale as the flat icon.** The layer is 108dp but only the central 72dp is ever shown (66dp guaranteed), so the logo circle is sized separately per target (`_flatCircleFraction` vs `_adaptiveCircleFraction`); and the foreground layer is additionally drawn inside a 16% `inset` that `flutter_launcher_icons` writes into the XML, so it carries `_foregroundInsetCompensation` to land on the circle behind it. Change one of those constants without the other and the two layers stop lining up.
