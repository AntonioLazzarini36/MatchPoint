...
15. ✅ **Identidad, firma y push (2026-08-21/22; id corregido el 2026-08-29)**: applicationId propio — era `com.matchpoint.app` y **hubo que cambiarlo a `es.matchpoint.tenis`**, porque el primero ya está ocupado en Google Play por otra app y los identificadores son únicos en toda la tienda: publicar con él era imposible. Se descubrió de casualidad al abrir un enlace de invitar construido a partir del id. Keystore de release propio, nombre e icono derivados del logo, y notificaciones push por FCM probadas en producción con la app cerrada. Cola de moderación (`/admin/reports`) y rate limiting por usuario fuera de auth.
